...
Removes noise and artifacts to generate clean masks with target pure colors.
"""
...
class MaskCleaner:
    ...
    def color_clustering(
        ...
        Cluster all colors in the image to specified number of pure colors.
        
        Args:
            image: Input image (BGR, uint8)
            num_colors: Number of colors to cluster
            target_colors: Target color list (if provided, will map clustering results to these colors)
    ...
    def nearest_color_mapping(
        ...
        Args:
            ...
            target_colors: Target pure color list, e.g., [(0,0,0), (255,0,0), (0,255,0)]
    ...
    def morphology_clean(
        ...
        Args:
            ...
            operations: Operation sequence
                ...
                - 'open-close': Opening then closing
                - 'close-open': Closing then opening
    ...
    def channel_wise_clean(
        ...
        Process each color channel separately to remove colors close but not pure.
        
        Args:
            image: Input image (BGR, uint8)
            target_colors: Target pure color list
...
def extract_dominant_colors(image: np.ndarray, num_colors: int = 5) -> List[Tuple[int, int, int]]:
    ...
    Returns:
        List of dominant colors (BGR format)
    """
    pixels = image.reshape(-1, 3).astype(np.float32)
...
def clean_binary_mask(
    ...
    """
    Clean binary mask (specifically for object detection).
    ...
    Args:
        ...
        min_object_size: Minimum object size (pixels), connected components smaller than this will be removed
...
def clean_instance_mask(
    ...
    Args:
        ...
        target_colors: Target color list, e.g., [(0,0,0), (0,0,255), (0,255,0), (255,0,0)]
        method: Cleaning method
