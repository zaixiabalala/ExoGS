...
def print_cuda_memory_usage():
    """Print CUDA memory usage for debugging."""
    import gc
...
def deserialize_json_into_object(file_path: Path, default: Any = None) -> Any:
    """Deserialize a JSON file into a Python object. Return default if file doesn't exist."""
    if file_path.exists():
...
def get_dtype_from_parameters(module: torch.nn.Module) -> torch.dtype:
    """Get a module's parameter dtype by checking one of its parameters."""
    return next(iter(module.parameters())).dtype


def get_output_shape(module: torch.nn.Module, input_shape: tuple) -> tuple:
    """Calculate the output shape of a PyTorch module given an input shape."""
...
def regional_sum(arr: np.ndarray, start_ends: list[tuple[int, int]]) -> np.ndarray:
    """Compute regional sum of array."""
    return np.stack([arr[..., s:e].sum(axis=-1) for s, e in start_ends], axis=-1)


def symmetric_regional_sum(arr: np.ndarray, start_ends: list[tuple[int, int]]) -> np.ndarray:
    """Compute symmetric regional sum of array."""
    return np.stack([np.stack([arr[..., s2:e2, s1:e1].sum(axis=(-1, -2)) for s2, e2 in start_ends], axis=-1) for s1, e1 in start_ends], axis=-1)
